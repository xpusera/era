...
def parse_header(filepath):
    ...
    for class_match in class_matches:
        ...
        # Find the end of the class/struct block (very simplified)
        ...
        public_content = public_sections[0] # Usually public by default in structs
        ...
        # Matches: void methodName(...) ;
        method_matches = re.findall(r'\b([A-Za-z0-9_]+)\s*\([^)]*\)\s*(?:const)?\s*(?:override|final)?\s*;', public_content)
        for method in method_matches:
            if method not in ('if', 'for', 'while', 'switch', 'return', 'catch', 'throw'):
                entities.append(f"{class_snake}.{camel_to_snake(method)}")

        # Extract properties
        # Matches: type propertyName;
        prop_matches = re.findall(r'\b(?:[A-Za-z0-9_<>:*]+)\s+([A-Za-z0-9_]+)\s*=', public_content) # initialization
        prop_matches += re.findall(r'\b(?:[A-Za-z0-9_<>:*]+)\s+([A-Za-z0-9_]+)\s*;', public_content) # declaration
        for prop in prop_matches:
            if prop not in ('public', 'private', 'protected', 'virtual', 'const', 'static', 'return'):
...
def main(src_dir, output_file):
    ...
    for root, dirs, files in os.walk(src_dir):
        ...
        if 'myengine' in root or 'benchmark' in root or 'unittest' in root:
            continue
        for file in files:
            if file.endswith('.h'):
